...
class ModernUI(BaseUI):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(top_frame, text="Search:", font=("Segoe UI", 13)).pack(side="left")
        ...
        ctk.CTkCheckBox(top_frame, text="Search archives", variable=self._archive_var,
                        command=self._on_search, font=("Segoe UI", 11)).pack(side="left")
        ...
        ctk.CTkLabel(right_frame, text="Details", font=("Segoe UI", 14, "bold"),
                     text_color="#aaa").pack(anchor="w", padx=12, pady=(10, 2))

        self._detail_info = ctk.CTkLabel(right_frame, text="", font=("Segoe UI", 11),
                                          text_color="#999", justify="left", anchor="w")
        self._detail_info.pack(anchor="w", padx=12, fill="x")

        ctk.CTkLabel(right_frame, text="Preview", font=("Segoe UI", 13, "bold"),
        ...
        ctk.CTkButton(btn_frame, text="📌 Unpin", fg_color="#e8a838",
        ...
        # ── Status bar ───────────────────────────────────────────
        self._status_label = ctk.CTkLabel(app, text="", font=("Segoe UI", 10),
                                           text_color="#888")
        self._status_label.pack(fill="x", padx=12, pady=(0, 6))

    def bind_events(self):
        self._app.bind("<Return>", lambda e: self.copy_selected())
        self._app.bind("<Delete>", lambda e: self.delete_selected())
        self._app.bind("<Control-p>", lambda e: self.pin_selected())
        self._app.bind("<Control-f>", lambda e: self._focus_search())
        self._app.bind("<Escape>", lambda e: self.hide())
    ...
    def _render_section_header(self, title: str):
        ctk.CTkLabel(self._list_frame, text=title, font=("Segoe UI", 11, "bold"),
    ...
    def _render_item(self, item: dict):
        ...
        label = ctk.CTkLabel(row, text=preview, font=("Segoe UI", 11),
        ...
        if item.get("pinned"):
            meta += " | 📌"
        meta_label = ctk.CTkLabel(row, text=meta, font=("Segoe UI", 9),
    ...
    def _update_detail(self):
        ...
        self._detail_info.configure(text=info)
